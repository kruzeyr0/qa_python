...
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "postgresql+psycopg2://admin:admin@localhost:5432/hillel_db"
)
...
def create_student(session, student_name, courses):
    ...
    count = min(len(courses), random.randint(1, 2))
    student.courses = random.sample(courses, count)  # випадковим чином призначаємо студенту від 1 до 2 курсів
    session.add(student)
    session.commit()
...
if __name__ == "__main__":
    ...
    student_names = ['Andrii', 'Masha', 'Alex', 'Tanya', 'Sergii']
    course_name = ['Java', 'Python', 'JavaScript']
    ...
    # Прінт інформації про студента за ім'ям або курсу за ім'ям
    print("\n--- Student Info ---")
    ...
    print("\n--- Check Deleted ---")
    print_student_by_name(session, "Alex")
