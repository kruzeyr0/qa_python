...
search_criteria = (2017, 1.6, 36000)    # year>=2017, engine_volume>=1.6, price<=36000

def find_cars(car_data, search_criteria):   # function to find cars based on criteria
    year_min, engine_volume_min, price_max = search_criteria
    filtered_cars = [
        (car, details) for car, details in car_data.items()
        if details[1] >= year_min and details[2] >= engine_volume_min and details[4] <= price_max   # filter cars based on criteria
    ]
    def get_price(car_item):
        return car_item[1][4]
    sorted_cars = sorted(filtered_cars, key=get_price)  # sort cars by price ascending

    return sorted_cars[:5]  # return up to 5 cars

for car, details in find_cars(car_data, search_criteria):  # iterate through result cars
    print(f"{car}: Color: {details[0]}, Year: {details[1]}, Engine Volume: {details[2]}, Type: {details[3]}, Price: {details[4]}")  # print the result cars